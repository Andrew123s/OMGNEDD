...
namespace omg::ui
{
    ...
    */
    ...
    {
    public:
        EffectEngineSection (juce::AudioProcessorValueTreeState& state,
                             std::function<float()> driveReader)
            : OmgPanel ({}, true),
              apvts (state),
              modes (state, pid::engine, 3, true),
              character (state, pid::character, OmgKnob::Large),
              driveMeter (std::move (driveReader))
        {
        ...
        void resized() override
        {
            ...
            // the drive meter sits between CHARACTER and the engine's controls,
            // where it reads as a consequence of the knob above it
            driveMeter.setBounds (area.removeFromBottom (27)
                                      .withSizeKeepingCentre (juce::jmin (300, area.getWidth() * 2 / 3), 27));
