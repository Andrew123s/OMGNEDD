...
namespace omg::ui
{
    ...
    OmgDriveMeter::OmgDriveMeter (std::function<float()> reader) : read (std::move (reader))
    {
        setTooltip ("DRIVE\nHow hard the character engine is being hit, measured where the "
                    "signal is handed to it. Past about -12 dB the nonlinear stages are "
                    "working; past -3 dB they are the loudest thing in the chain.");
        startTimerHz (30);
    }

    void OmgDriveMeter::timerCallback()
    {
        const float db = read != nullptr ? read() : -100.0f;
        // fast up, slow down, the ballistics a hardware meter has
        shown = db > shown ? shown + (db - shown) * 0.55f : shown + (db - shown) * 0.12f;
        repaint();
    }

    void OmgDriveMeter::paint (juce::Graphics& g)
    {
        auto area = getLocalBounds();
        auto head = area.removeFromTop (13);

        Fonts::drawLabel (g, "DRIVE", head, 9.0f, col::ink, 0.10f, juce::Justification::centredLeft, true);

        g.setFont (Fonts::mono (11.0f));
        g.setColour (col::inkMuted);
        g.drawText (shown <= -59.0f ? "-inf" : juce::String (shown, 1) + " dB", head,
                    juce::Justification::centredRight, false);

        area.removeFromTop (2);
        auto trough = area.removeFromTop (juce::jmin (12, area.getHeight()));
        Chassis::drawRecess (g, trough.toFloat(), (float) metric::radiusMd);

        auto inner = trough.reduced (2);
        const float norm = juce::jlimit (0.0f, 1.0f, (shown + 60.0f) / 60.0f);
        const int w = juce::roundToInt (norm * (float) inner.getWidth());

        if (w > 0)
        {
            g.setColour (shown > -3.0f ? col::clip : shown > -12.0f ? col::warning : col::accent);
            g.fillRect (inner.getX(), inner.getY(), w, inner.getHeight());
        }
    }
}
