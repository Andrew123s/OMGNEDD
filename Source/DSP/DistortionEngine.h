...
namespace omg::dsp
{
    /** Eleven nonlinear algorithms with pre-emphasis, bias, asymmetry, bit and
    ...
    public:
        ...
        static float shape (int algo, float x, float edge)
        {
            switch (algo)
            {
                ...
                case Shaper:  { // a Chebyshev-weighted polynomial: third and fifth
                                // order content on top of the fundamental, which
                                // reads as presence rather than dirt on a vocal
                                const float c = juce::jlimit (-1.0f, 1.0f, x);
                                const float t3 = 4.0f * c * c * c - 3.0f * c;
                                const float t5 = 16.0f * c * c * c * c * c - 20.0f * c * c * c + 5.0f * c;
                                return juce::jlimit (-1.0f, 1.0f, c + edge * (0.45f * t3 + 0.22f * t5)); }
                case BitCrush: { // a pure quantiser, as an algorithm rather than
                                 // the CRUSH knob's blended reduction
                                 const float levels = juce::jmax (2.0f, std::round (juce::jmap (edge, 48.0f, 3.0f)));
                                 return juce::jlimit (-1.0f, 1.0f, std::round (x * levels) / levels); }
                case Asym:    { // a different curve either side of zero, which
                                // generates the even harmonics a valve stage does
                                return x >= 0.0f ? std::tanh (x * (1.0f + edge * 2.2f))
